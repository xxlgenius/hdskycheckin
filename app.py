...
def job(userName:str,userPwd:str,headless:bool,web:str,webaddr:str):
    print(time.ctime()+"/开始执行")
    loginurl = 'https://hdsky.me/login.php'
    driver = None
    webOptions = None
    if web == "chrome":
        print(time.ctime()+"/开始注册chrome"+"   /浏览器地址:"+webaddr )
        webOptions = webdriver.ChromeOptions()
        if(headless):
            print(time.ctime()+"/无头模式启动")
            webOptions.add_argument('--headless')
            webOptions.add_argument("--disable-gpu")
        #driver = webdriver.Chrome(options=webOptions)
    elif web == "edge":
        print(time.ctime()+"/开始注册edge"+"   /浏览器地址:"+webaddr )
        ...
        print(time.ctime()+"/开始注册firefox"+"   /浏览器地址:"+webaddr)
    ...
    print(time.ctime()+"/开始连接远程浏览器")
    driver = webdriver.Remote(command_executor=webaddr,options=webOptions)
    print(time.ctime()+"/浏览器注册完成")


    driver.delete_all_cookies()
    # 打开网页
    driver.get(loginurl)
    print(time.ctime()+"/打开目标网页")
    '''
    time.sleep(5)
    # cookies注入
    ...
    print(time.ctime()+"/验证码解析完毕：" + log_code)
    ...
    print(time.ctime()+"/完成登录")

    try:
        driver.find_element(By.XPATH,'//*[@id="showup"]').click()
    except:
        print(time.ctime()+'/未找到目标元素，已签到')
    ...
    try:
        ...
        print(time.ctime()+"/签到验证码解析完毕："+check_code)
        #print(check_code)
    try:
        driver.find_element(By.XPATH,'//*[@id="imagestring"]').send_keys(check_code)
        driver.find_element(By.XPATH,'//*[@id="showupbutton"]').click()
        #print(time.ctime() +"/debug:输入验证码完毕并点击了提交")
    except:
        print(time.ctime() +"/r签到失败，没找到提交面板")
        sendError(time.ctime() + "/r签到失败，没找到提交面板")
    else:
        print(time.ctime()+"/签到成功")

    driver.quit()

def dosth():
    print(time.ctime()+"/woooeee")
...
def sendError(sendcontent:str):
    ...
    try:
        with open(smtpjson,'r',encoding='utf-8') as file:
            ...
            print(time.ctime()+"/获取邮件配置文件成功")
        maildriver = mysmtp.Smtpmailserver(mymail)
    except:
        print(time.ctime()+"/未设定邮箱或邮箱格式出错")
        return
    maildriver.Sendmail(sendcontent)
    print("通知邮件发送成功")
    
def do_schedule():
    print("正在运行计划任务, 每天签到一次")

    while True:
        print("距离下一次任务的时间: {:.1f}分钟".format(schedule.idle_seconds()), end='\r')
