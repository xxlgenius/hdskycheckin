...
def get_captcha(img):
    ...
    list = []
    templates_path = os.path.join(os.getcwd(),"app",'captcha', 'split','templates')
    #templates_path = templates_path.replace('\\', '/')
    #templates_path = os.path.normcase(templates_path)
    #templates_path = os.path.abspath('.')+'captcha'+'split'+'templates'
    print(time.ctime() + templates_path)
...
def binary_captchar(pathname):
    ...
    for left in left_list:
        for box in boxes:
            ...
            print(time.ctime()+"/删除了临时文件")

    return captcha


# if __name__ == '__main__':
    # captcha_floder = os.path.join(
    # os.path.abspath('.'), 'captcha', 'captcha')  # 测试
